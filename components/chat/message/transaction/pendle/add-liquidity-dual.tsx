...
import { PendleAddLiquidityDualTransaction } from "@/types/pendle-types";
...
interface Props {
  tx: PendleAddLiquidityDualTransaction;
...
const PendleAddLiquidityDual: React.FC<Props> = ({
  tx,
  setMessages,
  messages,
  client,
  ...
  const handleAddLiquidityDual = useCallback(async () => {
    try {
      // Set up approvals for both token and PT token
      const tokenApprovals = [
        // Token approval
        {
          tokenAddress: tx.token_address, 
          spender: tx.transaction.to,
          amount: tx.amount_token_in
        },
        // PT token approval
        {
          tokenAddress: tx.pt_address,
          spender: tx.transaction.to,
          amount: tx.amount_pt_in,
        }
      ];
      ...
      if (result.error) {
        ...
        try {
          ...
          if (txReceipt.status === "reverted") {
            ...
            const confirmMessage = `## Dual Liquidity Added to Pendle Successfully 🚀\n\n**Transaction Details:**\n- **Market:** ${tx.market}\n- **Token:** ${tx.token_in} (${formatAmount(tx.amount_token_in, tx.token_decimals)})\n- **PT Token:** ${tx.pt_token} (${formatAmount(tx.amount_pt_in, tx.pt_decimals)})\n- **LP Tokens Received:** ${formatAmount(tx.lp_tokens_received)}\n- **Status:** Confirmed ✅\n- **Hash:** [${result.hash.slice(0, 10)}...${result.hash.slice(-8)}](https://sonicscan.org/tx/${result.hash})`;
      ...
      console.error("Error executing Pendle dual liquidity transaction:", error);
    ...
    <Card className="w-full sm:w-[420px] md:w-[540px] lg:w-[640px] shadow-xl rounded-2xl overflow-hidden border-0 bg-gradient-to-b from-green-950 to-slate-950">
      <CardHeader className="py-5 px-6 border-b border-green-900/30 text-center bg-green-950/80 relative overflow-hidden">
        ...
        <CardTitle className="text-2xl font-semibold tracking-wide font-mono text-white relative z-10 flex items-center justify-center gap-2">
          ...
          <span>Add Dual Liquidity to Pendle</span>
        </CardTitle>
      </CardHeader>
      ...
      <CardContent className="p-6 space-y-6">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 border-b border-green-900/30 pb-4 group transition-all duration-300 hover:bg-green-900/20 p-2 rounded-md">
            ...
            <div className="flex flex-col flex-1 min-w-0">
              <span className="text-white/70 text-sm mb-1">PT Token In</span>
              <span className="font-semibold text-white truncate">
                {tx.pt_token}
              </span>
              <span className="text-sm text-green-300 mt-1">
...
export default PendleAddLiquidityDual;
