...
import { PendleRedeemSyTransaction } from "@/types/pendle-types";
...
const PendleRedeemSy: React.FC<Props> = ({
  tx,
  setMessages,
  messages,
  client,
  ...
  const handleRedeemSy = useCallback(async () => {
    try {
      // For redeeming SY, we need to approve the SY tokens
      const tokenApprovals = [{
        tokenAddress: tx.sy_address,
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 border-b border-green-900/30 pb-4 group transition-all duration-300 hover:bg-green-900/20 p-2 rounded-md">
            ...
            <div className="flex flex-col flex-1 min-w-0">
              <span className="text-white/70 text-sm mb-1">SY Tokens to Redeem</span>
              <span className="font-semibold text-white truncate">
                SY-{tx.sy_symbol}
              </span>
              <span className="text-sm text-green-300 mt-1">
                Amount: {formatAmount(tx.amount_in, tx.sy_decimals)}
              </span>
            </div>
          </div>
